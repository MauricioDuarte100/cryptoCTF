# DGHV Approximate GCD Solver - Improved Version
...
def chen_nguyen_attack(ciphertexts, t=5):
    """
    ...
    rho = noise_bits + 10  # Shift parameter
...
def multivariate_coppersmith(ciphertexts, num=8):
    """
    ...
    print(f"[*] Running BKZ on {n+1}x{n+1} Coppersmith lattice...")
    try:
        L = M.BKZ(block_size=min(25, n+1))
    except:
        L = M.LLL()
...
def howgrave_graham_attack(ciphertexts, num=6):
    """
    ...
    for i in range(1, n+1):
        M[i, 0] = ciphertexts[i] if i < len(ciphertexts) else 0
        M[i, i] = 1
    
    print(f"[*] Running LLL on {n+1}x{n+1} Howgrave-Graham lattice...")
    L = M.LLL()
...
def ratio_lattice(ciphertexts, num=5):
    """
    ...
    remainders = [ciphertexts[i] % c0 for i in range(1, n)]
    ...
    for r in remainders:
        if r != 0:
            g = gcd(c0, r)
            bits = g.nbits()
            if 100 <= bits <= 150 and is_prime(g):
                candidates.append(g)
...
# Decrypt
def decrypt_and_verify(p, cts, mod=128):
    noise_bound = 2^127
    for ct in cts[:5]:
        if ct % p > noise_bound:
            return None
