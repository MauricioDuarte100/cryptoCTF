#!/usr/bin/env python3
"""
Size-Limit Challenge Solver v2
==============================
Intentar diferentes interpretaciones del plaintext descifrado.
"""

from Crypto.Util.number import long_to_bytes
import zlib
import gzip
import base64

# Datos del challenge
N = 
e = 65537
c = 
d = 

# Descifrar
m = pow(c, d, N)
flag_bytes = long_to_bytes(m)

print(f"[*] Length: {len(flag_bytes)} bytes")
print(f"[*] Hex: {flag_bytes.hex()}")
print()

# 1. Intentar zlib decompress
try:
    decompressed = zlib.decompress(flag_bytes)
    print(f"[+] ZLIB decompressed: {decompressed}")
except:
    print("[-] Not zlib compressed")

# 2. Intentar gzip
try:
    import io
    decompressed = gzip.decompress(flag_bytes)
    print(f"[+] GZIP decompressed: {decompressed}")
except:
    print("[-] Not gzip compressed")

# 3. Quizás el hex ES el flag (un string de 256 caracteres hex)
hex_str = flag_bytes.hex()
print(f"\n[*] Hex string as potential flag (length {len(hex_str)}): {hex_str[:100]}...")

# 4. Intentar interpretar como ASCII rotado/xoreado
for key in range(256):
    xored = bytes([b ^ key for b in flag_bytes])
    if xored.startswith(b'RTACTF{') or xored.startswith(b'flag{') or xored.startswith(b'CTF{'):
        print(f"[+] XOR key {key}: {xored}")
        break

# 5. Verificar si cada byte es un carácter printable después de un shift
# Por ejemplo, si cada byte tiene un offset de 0x30 o similar
for offset in range(-128, 128):
    shifted = bytes([(b + offset) % 256 for b in flag_bytes])
    try:
        decoded = shifted.decode('ascii')
        if decoded.isprintable():
            print(f"[+] Shift {offset}: {decoded}")
    except:
        pass

# 6. Mostrar como lista de valores para inspección
print(f"\n[*] First 20 bytes as ints: {list(flag_bytes[:20])}")

# 7. Quizás el flag REAL está en el archivo flag.py y esto es un troll?
# Pero el problema dice que la longitud es 131, así que debería estar aquí.
print(f"\n[*] Trying to find any printable sequence...")
import re
printable_seq = re.findall(rb'[\x20-\x7e]{5,}', flag_bytes)
for seq in printable_seq:
    print(f"[+] Printable sequence: {seq}")
